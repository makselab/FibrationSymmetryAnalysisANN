...
class PPO(BaseLoss):
    ...
    def run_rollout(self, td: TensorDict, trainer_state: TrainerState) -> None:
        with torch.no_grad():
            # self.policy(td)
            self.policy._batch_dim(td)
            self.policy(td)
            self.policy._get_action(td)
    ...
    def run_train(self, shared_loss_data: TensorDict, trainer_state: TrainerState) -> tuple[Tensor, TensorDict]:
        ...
        policy_td = self.policy._batch_dim(policy_td)
        self.policy(policy_td)
        policy_td = self.policy._get_action(policy_td, action=minibatch["actions"])
