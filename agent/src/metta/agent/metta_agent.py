...
class MettaAgent(nn.Module):
    ...
    # KCore ========================================
    def _batch_dim(self, td: TensorDict):
        return self.policy._batch_dim(td)

    def _get_action(self, td: TensorDict, action: Optional[torch.Tensor] = None) -> TensorDict:
        if action is None:
            output_td = self.policy.forward_inference(td)
        else:
            output_td = self.policy.forward_training(td, action)
